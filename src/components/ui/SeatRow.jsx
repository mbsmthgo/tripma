...
export default function SeatRow({row, rowNumber, selected, setSelected}) {
    if (rowNumber < 6) {
        return (
            <>
                {rowNumber === 1 ? <rect width={200} height={312} x={1113} y={614} fill="#fff" rx={8}/> : null}
                <BusinessRow row={row} rowNumber={rowNumber} selected={selected} setSelected={setSelected} />
            </>
        )
    } else {
        return (
            <>
                ...
                <EconomyRow row={row} rowNumber={rowNumber} selected={selected} setSelected={setSelected} />
            </>
        )
    }
}